...
class Shaft:
    ...
    def check_torsional_resonance(self, evaluate_x: float, gear_inertia_kgm2: float, operating_rpm: float, num_teeth: int, safety_margin_percent: float = 20.0):
        """
        Checks if the motor's operating speed risks hitting the shaft's natural torsional frequency.
        """
        import math
        
        twist_rad = abs(self.get_torsion_angle_at(evaluate_x))
        
        if twist_rad == 0:
            return False, "No twist detected. Stiffness is effectively infinite."

        # self.torque is (start, end, mag) in N*mm. We need N*m for the frequency formula.
        torque_Nm = abs(self.torque[2]) / 1000.0  
        K_stiffness = torque_Nm / twist_rad

        if gear_inertia_kgm2 <= 0:
            raise ValueError("Gear inertia must be greater than 0.")
            
        # 1. Calculate Natural Frequency
        natural_freq_hz = (1.0 / (2 * math.pi)) * math.sqrt(K_stiffness / gear_inertia_kgm2)
        
        # 2. Calculate Excitation Frequencies
        shaft_freq_hz = operating_rpm / 60.0
        gear_mesh_freq_hz = shaft_freq_hz * num_teeth

        # 3. Define Danger Bounds
        lower_bound = natural_freq_hz * (1.0 - (safety_margin_percent / 100.0))
        upper_bound = natural_freq_hz * (1.0 + (safety_margin_percent / 100.0))

        # 4. Check if ANY excitation is in the danger zone
        shaft_in_danger = lower_bound <= shaft_freq_hz <= upper_bound
        mesh_in_danger = lower_bound <= gear_mesh_freq_hz <= upper_bound
        is_in_danger = shaft_in_danger or mesh_in_danger

        # 5. Format Report
        if is_in_danger:
            status = "⚠️ WARNING: RESONANCE DANGER"
            if mesh_in_danger:
                status += " (Driven by Gear Mesh)"
            elif shaft_in_danger:
                status += " (Driven by 1X RPM Excitation)"
        else:
            status = "✅ SAFE: Outside resonance zone"
        
        report = (
            f"--- Resonance Diagnostics ---\n"
            f"Stiffness (K)     : {K_stiffness:.2f} Nm/rad\n"
            f"Natural Freq (fn) : {natural_freq_hz:.2f} Hz\n"
            f"1X Shaft Freq     : {shaft_freq_hz:.2f} Hz\n"
            f"Gear Mesh Freq    : {gear_mesh_freq_hz:.2f} Hz\n"
            f"Danger Zone       : {lower_bound:.2f} Hz to {upper_bound:.2f} Hz\n"
            f"Status            : {status}"
        )
        return is_in_danger, report
    ...
    def plot_shear_bending_diagrams(self, axis, num_points=1000):
        ...
        ax1.plot(x_vals, V_vals, color='steelblue', linewidth=2)
        ax1.fill_between(x_vals, V_vals, 0, color='steelblue', alpha=0.2)
    ...
    def torque_diagram(self):
        ...
        plt.show()
def calculate_system_phase_shift(*stages):
    """
    Calculates the total windup across an undetermined number of shafts.
    Args:
        *stages: Tuples of (Shaft_object, evaluation_x, gear_ratio_to_next)
                 Ratio = (Driving Pitch Diameter / Driven Pitch Diameter)
    """
    current_windup = 0.0
    ...
    for stage in stages:
        ...
        # 1. Add this shaft's physical twist
        twist_here = abs(shaft.get_torsion_angle_at(eval_x))
        current_windup += twist_here

        # 2. Scale by the gear ratio if it exists
        if len(stage) == 3 and stage[2] is not None:
            ratio_to_next = stage[2]
            current_windup *= ratio_to_next

    return current_windup